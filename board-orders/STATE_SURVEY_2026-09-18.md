...
|---|---|---|---|
| Delaware | Open-data CSV https://data.delaware.gov/api/views/dz6p-akeq/rows.csv?accessType=DOWNLOAD (all DPR disciplinary actions; Professional Counselor of Mental Health = 33 people, 69 action rows, 1997 to 2025) | DELPROS (Salesforce): each license record lists its Board Orders with public links; the search and the document list answer JavaScript remoting calls with no CAPTCHA (verified 2026-09-19 on one licensee, 3 orders listed). The file download runs through a Salesforce viewer and was not reproduced by script | MODERATE (verified 2026-09-19). Qualifies under the public-listing rule; about 30 to 40 documents; not built yet. See SURVEY_VERIFICATION_DE_OR_AK_NE_2026-09-19.md. |
| Oregon | The cumulative PDF is gone (404 live and in the archive); no list is posted | Thentia register https://oblpct.us.thentiacloud.net/webs/oblpct/register/ : each record's public notices (Notice of Proposed Action, Default Order, Stipulated Order, Bill of Costs) carry PDF attachments served by plain GET, same REST design as Oklahoma (verified 2026-09-19, one order downloaded). Search results omit the notices, so the index needs one record call per LPC and associate (about 19,500 calls) or a status-only shortcut (Revoked, Surrendered, Suspended: 114 people) | MODERATE (verified 2026-09-19). SKIPPED by decision 2026-09-19: no posted list, and a complete index would need a sweep of about 19,500 records. See SURVEY_VERIFICATION_DE_OR_AK_NE_2026-09-19.md. |
| California | BBS recent-actions list https://www.bbs.ca.gov/consumers/enforcement_actions.html (one month of names only, no links, no archive; LPCC labeled, 0 to 2 a month; older months only in the Wayback Machine) | DCA License Search https://search.dca.ca.gov/ per licensee: Advanced Search has Has Discipline = Yes and Has Documents = Yes filters and the detail pages link the Accusation, Stipulation and Decision PDFs, but the search form is a Cloudflare Turnstile CAPTCHA, detail and download addresses carry opaque tokens, and robots.txt disallows /details/ and /download. The scripted alternative is DCA's iServices Search API (https://iservices.dca.ca.gov/, registered key approved by DCA required) | HARD (verified 2026-09-19, down from MODERATE). SKIPPED by decision 2026-09-19: the project takes only orders listed publicly on a board website, and California's are behind a CAPTCHA-gated search. No downloader; API and PRA routes noted for reference only. See CALIFORNIA_VERIFICATION_2026-09-19.md. |
| Alaska | Quarterly all-profession PDFs, predictable URLs https://www.commerce.alaska.gov/web/Portals/5/pub/LicActions{YYYY}Qtr{N}.pdf (2017 on), sectioned by board, one summary paragraph per action | Search Professional Licenses portal per licensee | NOT FEASIBLE by script (verified 2026-09-19): the whole commerce.alaska.gov host sits behind a DataDome bot challenge (HTTP 403 to scripts); the PDFs are readable only via the Wayback Machine and the per-licensee orders not at all. SKIPPED. See SURVEY_VERIFICATION_DE_OR_AK_NE_2026-09-19.md. |
| Nebraska | Monthly PDFs https://dhhs.ne.gov/licensure/Documents/MM-YYdiscip.pdf plus a rolling ten-year file (August16-26discip.pdf), PROFESSION column; 23 Mental Health Practitioner rows in ten years | LISSearch lookup per licensee (documents in the 'Disciplinary/Non-Disciplinary Information' section) | NOT FEASIBLE by script (verified 2026-09-19): the LIS search form carries a Google reCAPTCHA. Index only. SKIPPED. See SURVEY_VERIFICATION_DE_OR_AK_NE_2026-09-19.md. |
