...
|---|---|---|---|
| Colorado | DORA roster generator https://apps2.colorado.gov/dora/licensing/lookup/generateroster.aspx exports a licensee/discipline list by license type (LPC) with case number, action, dates | Per-licensee ASP.NET lookup, documents attached since 1999/2000 | MODERATE. Roster gives the exact list of who to fetch; a second script then pulls each licensee's documents. |
...
| Oklahoma | Legacy page https://www.ok.gov/behavioralhealth/License_Verification.html described per-license-type "Disciplinary Licensee" lists linking Board Orders; may have moved to the Thentia register https://obbhl.us.thentiacloud.net/webs/obbhl/register/ | Possibly still linked from the disciplinary list; AG board supervisory letters (HTML) summarize each action as a fallback | MODERATE. If the legacy list still works it moves to Tier 1. On the handoff candidate list. Verified 2026-09-18: legacy ok.gov page is gone (redirects to oklahoma.gov/behavioralhealth.html, no discipline list). The Thentia register's REST layer answers plain GET requests without a browser: `rest/public/profile/search/?keyword=&take=2000&disciplined=true&profession=Licensed%20Professional%20Counselor%20(LPC)` lists 144 disciplined LPCs (186 all types); `rest/public/profile/get/?id=` returns each Public Notice with a signed PDF download link (`rest/public/annotation/download/index.php?id=&entity=`). Notices back to 1999, attached orders only from about 2021 on (roughly 70 to 90 LPC PDFs); 3 of 3 samples are scans, OCR needed. AG letters are JS-listed HTML summaries, fallback only. EASY; moves to Tier 1. See SURVEY_VERIFICATION_OK_NM_NV_ND_2026-09-18.md. |
...
| State | Why |
|---|---|
| Texas | BHEC publishes discipline only through the HPC datamart lookup (https://vo.licensing.hpc.texas.gov/datamart/), a Java form portal with per-licensee drill-down. No standalone list found. Largest LPC population in the country, so worth confirming on bhec.texas.gov whether an enforcement list exists; Council meeting-materials PDFs list agreed orders quarterly as a partial fallback. |
...
6. Colorado and Delaware as the first two-step states (roster export, then per-licensee document pull), if the project wants larger western coverage.
