...
| Oregon | The cumulative PDF is gone (404 live and in the archive); no list is posted | Thentia register https://oblpct.us.thentiacloud.net/webs/oblpct/register/ : each record's public notices (Notice of Proposed Action, Default Order, Stipulated Order, Bill of Costs) carry PDF attachments served by plain GET, same REST design as Oklahoma (verified 2026-09-19, one order downloaded). Search results omit the notices, so the index needs one record call per LPC and associate (about 19,500 calls) or a status-only shortcut (Revoked, Surrendered, Suspended: 114 people) | MODERATE (verified 2026-09-19). Qualifies under the public-listing rule; not built yet. See SURVEY_VERIFICATION_DE_OR_AK_NE_2026-09-19.md. |
